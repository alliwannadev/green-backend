package alliwannadev.shop.core.jpa.product.repository;

import alliwannadev.shop.core.domain.common.dto.InfiniteScrollCond;
import alliwannadev.shop.core.jpa.common.util.QuerydslUtil;
import alliwannadev.shop.core.jpa.product.repository.dto.GetProductDto;
import alliwannadev.shop.core.jpa.product.repository.dto.GetProductListCond;
import alliwannadev.shop.core.jpa.product.repository.dto.QGetProductDto;
import com.querydsl.core.types.Predicate;
import com.querydsl.core.types.dsl.BooleanExpression;
import com.querydsl.jpa.impl.JPAQueryFactory;
import lombok.RequiredArgsConstructor;
import org.apache.commons.lang3.ObjectUtils;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.data.support.PageableExecutionUtils;
import org.springframework.stereotype.Repository;
import org.springframework.transaction.annotation.Transactional;

import java.util.List;

import static alliwannadev.shop.core.jpa.option.model.QProductOptionCombinationEntity.productOptionCombinationEntity;
import static alliwannadev.shop.core.jpa.product.model.QProductEntity.productEntity;

@RequiredArgsConstructor
@Repository
public class ProductQueryJpaRepository {

    private final JPAQueryFactory primaryQueryFactory;

    @Transactional(readOnly = true)
    public Page<GetProductDto> findAll(
            GetProductListCond condition,
            Pageable pageable
    ) {
        List<GetProductDto> content =
                primaryQueryFactory
                        .select(
                                new QGetProductDto(
                                        productEntity.productId,
                                        productEntity.productCode.max(),
                                        productEntity.productName.max(),
                                        productEntity.modelName.max(),
                                        productEntity.originalPrice.max(),
                                        productEntity.sellingPrice.max(),
                                        productEntity.description.max(),
                                        productEntity.isDisplayed.max()
                                )
                        )
                        .from(productOptionCombinationEntity)
                        .join(productOptionCombinationEntity.product, productEntity)
                        .where(
                                sellingPriceBetween(condition.priceRange()),
                                QuerydslUtil.orConditions(toSelectedOptionsMatchAgainstArray(condition.sizes())),
                                QuerydslUtil.orConditions(toSelectedOptionsMatchAgainstArray(condition.colors()))
                        )
                        .groupBy(productEntity.productId)
                        .orderBy(productEntity.productId.desc())
                        .offset(pageable.getOffset())
                        .limit(pageable.getPageSize())
                        .fetch();

        Long totalCount =
                primaryQueryFactory
                        .select(productEntity.productId.countDistinct())
                        .from(productOptionCombinationEntity)
                        .join(productOptionCombinationEntity.product, productEntity)
                        .where(
                                sellingPriceBetween(condition.priceRange()),
                                QuerydslUtil.orConditions(toSelectedOptionsMatchAgainstArray(condition.sizes())),
                                QuerydslUtil.orConditions(toSelectedOptionsMatchAgainstArray(condition.colors()))
                        )
                        .fetchOne();

        return PageableExecutionUtils.getPage(
                content,
                pageable,
                () -> ObjectUtils.isEmpty(totalCount) ? 0 : totalCount
        );
    }

    @Transactional(readOnly = true)
    public List<GetProductDto> findAllInfiniteScroll(
            String categoryPath,
            InfiniteScrollCond infiniteScrollCond
    ) {
        return primaryQueryFactory
                .select(
                        new QGetProductDto(
                                productEntity.productId,
                                productEntity.productCode,
                                productEntity.productName,
                                productEntity.modelName,
                                productEntity.originalPrice,
                                productEntity.sellingPrice,
                                productEntity.description,
                                productEntity.isDisplayed
                        )
                )
                .from(productEntity)
                .where(
                        productIdLt(infiniteScrollCond.lastCursor())
                )
                .orderBy(productEntity.productId.desc())
                .limit(infiniteScrollCond.getPageSizeWithOneAdded())
                .fetch();
    }

    private BooleanExpression productIdLt(Long productId) {
        return productId == null ? null : productEntity.productId.lt(productId);
    }

    private BooleanExpression sellingPriceBetween(GetProductListCond.PriceRange priceRange) {
        return ObjectUtils.isEmpty(priceRange) ?
                null :
                productEntity.sellingPrice.between(
                        priceRange.minPrice(),
                        priceRange.maxPrice()
                );
    }

    private Predicate[] toSelectedOptionsMatchAgainstArray(
            List<GetProductListCond.OptionCond> options
    ) {
        return options
                .stream()
                .map(option ->
                        QuerydslUtil.matchAgainst(
                                productOptionCombinationEntity.selectedOptions,
                                "%s_%s".formatted(option.optionCode(), option.optionValue())
                        )
                )
                .toArray(Predicate[]::new);
    }
}
